...
public class TransactionService {
    ...
    public TransactionService(TransactionRepository transactionRepository, WebClient.Builder webClientBuilder) {
        ...
        this.webClient = webClientBuilder.baseUrl("http://localhost:8080/api/accounts").build();
    }
                ...
                .flatMap(account -> {
                    ...
                    if ("DEPOSIT".equalsIgnoreCase(request.getTransactionType())) {
                        ...
                        if (request.getAmount().compareTo(initialBalance) > 0) {
                            return Mono.error(new IllegalArgumentException("Insufficient balance for the withdrawal"));
                        }
                    ...
                    return getTransactionResponseDTOMono(request, transaction, finalBalance);
    ...
    public Flux<Transaction> streamTransactions() {
        return transactionRepository.findWithTailableCursor();
    }
}
